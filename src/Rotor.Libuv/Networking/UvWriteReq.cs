...
namespace Rotor.Libuv.Networking
{
    ...
    {
        private readonly static Binding.uv_write_cb _uv_write_cb = (IntPtr ptr, int status) => UvWriteCb(ptr, status);
        ...
        {
            var requestSize = loop.Binding.req_size(Binding.RequestType.WRITE);
            var bufferSize = Marshal.SizeOf<Binding.uv_buf_t>() * BUFFER_COUNT;
            CreateMemory(
                loop.Binding,
                loop.ThreadId,
            ...
            object state)
        {
            try
            {
                ...
                var pBuffers = (Binding.uv_buf_t*)_bufs;
                if (nBuffers > BUFFER_COUNT)
                {
                    // create and pin buffer array when it's larger than the pre-allocated one
                    var bufArray = new Binding.uv_buf_t[nBuffers];
                    var gcHandle = GCHandle.Alloc(bufArray, GCHandleType.Pinned);
                    _pins.Add(gcHandle);
                    pBuffers = (Binding.uv_buf_t*)gcHandle.AddrOfPinnedObject();
                }
                ...
                for (var index = 0; index < nBuffers; index++)
                {
                    ...
                    pBuffers[index] = Binding.buf_init(
        ...
        private static void UvWriteCb(IntPtr ptr, int status)
        {
            ...
            if (status < 0)
            {
                req.Binding.Check(status, out error);
            }
