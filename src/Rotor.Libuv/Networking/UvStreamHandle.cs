...
namespace Rotor.Libuv.Networking
{
    ...
    {
        private readonly static Binding.uv_connection_cb _uv_connection_cb = (handle, status) => UvConnectionCb(handle, status);
        // Ref and out lamda params must be explicitly typed
        private readonly static Binding.uv_alloc_cb _uv_alloc_cb = (IntPtr handle, int suggested_size, out Binding.uv_buf_t buf) => UvAllocCb(handle, suggested_size, out buf);
        private readonly static Binding.uv_read_cb _uv_read_cb = (IntPtr handle, int status, ref Binding.uv_buf_t buf) => UvReadCb(handle, status, ref buf);
        ...
        private Func<UvStreamHandle, int, object, Binding.uv_buf_t> _allocCallback;
        ...
        public void ReadStart(
            Func<UvStreamHandle, int, object, Binding.uv_buf_t> allocCallback,
        ...
        public int TryWrite(Binding.uv_buf_t buf)
        {
        ...
        private static void UvConnectionCb(IntPtr handle, int status)
        {
            ...
            Exception error;
            status = stream.Binding.Check(status, out error);

            stream._listenCallback(stream, status, error, stream._listenState);
        }

        private static void UvAllocCb(IntPtr handle, int suggested_size, out Binding.uv_buf_t buf)
        {
            var stream = FromIntPtr<UvStreamHandle>(handle);
            try
            {
            ...
            {
                buf = stream.Binding.buf_init(IntPtr.Zero, 0);
                throw;
            }
        }

        private static void UvReadCb(IntPtr handle, int status, ref Binding.uv_buf_t buf)
        {
            var stream = FromIntPtr<UvStreamHandle>(handle);
